const Ads = {
  ...
  isAIT() { return !!window.__AIT__; },
  isCapacitor() { return !!window.Capacitor && !window.__AIT__; },
  ...
  resumeBanner() {
    ...
    this._bannerVisible = true;
    if (this.isAIT()) this._showAITBanner();
    else if (this.isCapacitor()) window.Capacitor?.Plugins?.AdMob?.resumeBanner();
  },
  ...
  _showAITRewarded() {
    return new Promise((resolve) => {
      ...
      if (!showFullScreenAd?.isSupported?.()) { resolve(true); return; }
      let rewarded = false;
      showFullScreenAd({
        options: { adGroupId: this._AIT_REWARDED_ID },
        onEvent: (e) => {
          ...
          if (e.type === 'dismissed') {
            if (loadFullScreenAd?.isSupported?.()) {
              loadFullScreenAd({ options: { adGroupId: this._AIT_REWARDED_ID }, onEvent: () => {}, onError: () => {} });
            }
            resolve(rewarded);
          }
  ...
  async _showAdMobRewarded() {
    ...
    try {
      ...
      return new Promise((resolve) => {
        let rewarded = false;
        let rl, dl;
        const cleanup = () => { rl?.remove(); dl?.remove(); };
        AdMob.addListener('onRewardedVideoAdReward', () => { rewarded = true; }).then(h => { rl = h; });
        AdMob.addListener('onRewardedVideoAdDismissed', () => { cleanup(); resolve(rewarded); }).then(h => { dl = h; });
